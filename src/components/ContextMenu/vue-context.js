...
export default {
  ...
  methods: {
    ...
    onKeydown(event) {
      ...
      if (key === keyCodes.ESC) {
        ...
        if (
          menuContainer &&
          menuContainer.getElementsByClassName("v-context")[0] !== this.activeSubMenu
        ) {
    ...
    openSubMenu(event) {
      ...
      let [elementTop, elementLeft] = this.positionMenu(bcr.top, bcr.right, subMenuElement)
  ...
  render() {
    ...
    // TODO: Ver (y arreglar) porque no  funcionan estas directivas si las quiero usar acá directamente. O sea, no cierra on click outside
    const directives = [
      // {
      //   name: "click-outside",
      //   value: this.close,
